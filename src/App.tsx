...
import {
  Home as HomeIcon, Search, Plus, ShoppingBag, User, Settings, Store, Globe,
...
const API = "/api/v1";
const SUPPORT_EMAIL = "support@mggestion.ht";
...
type StoreType = { id:string; name:string; description?:string; logoUrl?:string; bannerUrl?:string; location?:string; rating?:number; salesCount?:number; verified?:boolean; productCount?:number };
type UserType = { id:string; fullName:string; email:string; avatarUrl?:string; isSeller?:boolean; store?:StoreType|null };
...
function Header({user,lang,setLang,search,setSearch,onLogin,onAccount,t}:{user:UserType|null;lang:Lang;setLang:(x:Lang)=>void;search:string;setSearch:(x:string)=>void;onLogin:()=>void;onAccount:()=>void;t:any}){
...
function Nav({tab,setTab,user,onLogin,t}:{tab:Tab;setTab:(x:Tab)=>void;user:UserType|null;onLogin:()=>void;t:any}){
  ...
  return <nav className="fixed bottom-0 left-0 right-0 z-50 px-3 pb-3"><div className="mx-auto flex max-w-lg items-end justify-between rounded-[30px] border border-slate-200 bg-white/95 px-2 py-2 shadow-2xl backdrop-blur-xl">{item("accueil",t.home,<HomeIcon className="h-5 w-5"/>)}{item("explorer",t.explorer,<Search className="h-5 w-5"/>)}<button onClick={()=>user?setTab("vann"):onLogin()} className="relative -mt-7 flex h-16 w-16 items-center justify-center rounded-[24px] bg-gradient-to-br from-[#0B1F3A] via-[#2F5FFF] to-[#00C2D6] text-white shadow-2xl"><Plus className="h-7 w-7"/></button>{item("panye",t.cart,<ShoppingBag className="h-5 w-5"/>)}{item("parametre",t.settings,<User className="h-5 w-5"/>)}</div></nav>;
}
